...
public:
	...
	//n0 means the current 
	//n means the total world map
	int **n0_mk;
	int **n_mk;
	...
	//c m 
	int *rn_m;
	//store the character of the word
	int **w_mn;
	int **z_mn;
	int **c_mn;
	int **s_mn;
    ...
    double ** theta_mk; // theta: document-topic distributions, size M x K
    double ** phi_kv; // phi: topic-word distributions, size K x V
	double ** delta_mm;
	double * lambda_m; 
	...
	double beta_vector;
	double eta_vector;
	double alpha_lambda_n;
	double alpha_lambda_c;
    model() {
	set_default_values();
    }
    ...
    int save_model_delta(string filename);
	int save_model_lambda(string filename);
	...
	int sampling_s_wave(int m, int n);
	int sampling_k_wave_first(int m, int n);
	int sampling_r_wave(int m, int n);
	int sampling_k_wave_second(int m, int n,int c_mn);
    void compute_theta();
    void compute_phi();
	void compute_delta();
	void compute_lambda();
